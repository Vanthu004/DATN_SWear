...
const registerForPushNotificationsAsync = async () => {
  ...
  if (!userId) {
    console.warn('[Push] userId is missing, cannot register for push notifications');
    return;
  }
  ...
  try {
    ...
    // Lấy Expo token
    const expoProjectId = 'your-expo-project-id';
    ...
    if (expoPushToken) {
      try {
        ...
        console.log('[Expo] Token đã gửi lên server, response.status =', res.status);
      } catch (err) {
        console.error('[Expo] Lỗi khi gửi token lên server:', err?.response?.data ?? err.message);
      }
    } else {
      console.warn('[Expo] Không có expoPushToken');
    }
  } catch (expoError) {
    ...
    if (enabled) {
      ...
      if (fcmToken) {
        try {
          ...
          console.error('[FCM] Lỗi khi gửi token lên server:', err?.response?.data ?? err.message);
        }
      }
    }
  }
